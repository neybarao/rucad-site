...
- **JS:** mínimo. Client components só onde há interação (menu mobile, acordeão de FAQ, banner de cookies se necessário).
...
- **Performance:** fontes com `display: swap` e subset latin; CSS enxuto; sem libs pesadas de animação por padrão. Núcleo do ganho frente ao site atual.
